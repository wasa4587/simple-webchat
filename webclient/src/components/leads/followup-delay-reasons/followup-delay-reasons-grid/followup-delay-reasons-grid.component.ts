...
@Component({
	selector: 'followup-delay-reasons-grid',
	encapsulation: ViewEncapsulation.None,
	templateUrl: './followup-delay-reasons-grid.component.html',
	styleUrls: [
		'./followup-delay-reasons-grid.component.scss',
	],
	providers: [
...
export class FollowupDelayReasonsGridComponent {
	public listData: any;
	...
	constructor(private followUpDelayReasonProvider: FollowUpDelayReasonProvider) {
	}
   ...
   */
	...
	private getData() {
		...
		filter['page'] = 1;
		this.followUpDelayReasonProvider.getAll(filter).subscribe((res) => {
			if (res && res.body) {
				...
				if (res.body.items) {
					if (res.body.paging.firstPage !== res.body.paging.lastPage && res.body.items.length > 0) {
						...
						this.orderByNameOrder('description');
